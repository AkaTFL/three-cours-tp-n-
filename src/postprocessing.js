import * as THREE from "three";
import { EffectComposer } from "three/addons/postprocessing/EffectComposer.js";
import { RenderPass } from "three/addons/postprocessing/RenderPass.js";
import { ShaderPass } from "three/addons/postprocessing/ShaderPass.js";

export function createGodRayPipeline({ scene, camera, renderer, sunLight }) {
  const occlusionMaterial = new THREE.MeshBasicMaterial({ color: 0x000000 });
  const lightMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff });
  const occlusionRenderTarget = new THREE.WebGLRenderTarget(window.innerWidth, window.innerHeight);

  const godRayShader = {
    uniforms: {
      tDiffuse: { value: null },
      lightPosition: { value: new THREE.Vector2(0.5, 0.5) },
      exposure: { value: 0.6 },
      decay: { value: 0.95 },
      density: { value: 0.8 },
      weight: { value: 0.4 },
    },
    vertexShader: `
      varying vec2 vUv;
      void main() {
        vUv = uv;
        gl_Position = vec4(position, 1.0);
      }
    `,
    fragmentShader: `
      varying vec2 vUv;
      uniform sampler2D tDiffuse;
      uniform vec2 lightPosition;
      uniform float exposure;
      uniform float decay;
      uniform float density;
      uniform float weight;
      const int NUM_SAMPLES = 30;
      void main() {
        vec2 delta = (vUv - lightPosition) * density / float(NUM_SAMPLES);
        vec2 coord = vUv;
        float illuminationDecay = 1.0;
        vec4 color = vec4(0.0);
        for (int i = 0; i < NUM_SAMPLES; i++) {
          coord -= delta;
          float sampleValue = texture2D(tDiffuse, coord).r;
          color += vec4(sampleValue) * illuminationDecay * weight;
          illuminationDecay *= decay;
        }
        color /= float(NUM_SAMPLES);
        vec4 sceneColor = texture2D(tDiffuse, vUv);
        gl_FragColor = sceneColor + color * exposure;
      }
    `,
  };

  const composer = new EffectComposer(renderer);
  composer.addPass(new RenderPass(scene, camera));

  const godPass = new ShaderPass(godRayShader);
  composer.addPass(godPass);

  godPass.uniforms.exposure.value = 0.2;
  godPass.uniforms.weight.value = 0.08;
  godPass.uniforms.density.value = 0.5;
  godPass.uniforms.decay.value = 0.95;
  godPass.uniforms.tDiffuse.value = occlusionRenderTarget.texture;
  godPass.material.blending = THREE.AdditiveBlending;
  godPass.material.transparent = true;

  function renderOcclusion() {
    scene.overrideMaterial = occlusionMaterial;

    const sunSphere = new THREE.Mesh(new THREE.SphereGeometry(200, 16, 16), lightMaterial);
    sunSphere.position.copy(sunLight.position);
    scene.add(sunSphere);

    renderer.setRenderTarget(occlusionRenderTarget);
    renderer.clear();
    renderer.render(scene, camera);

    scene.remove(sunSphere);
    scene.overrideMaterial = null;
    renderer.setRenderTarget(null);
  }

  function render() {
    const sunScreenPos = sunLight.position.clone().project(camera);
    godPass.uniforms.lightPosition.value.set((sunScreenPos.x + 1) / 2, (sunScreenPos.y + 1) / 2);
    renderOcclusion();
    godPass.uniforms.tDiffuse.value = occlusionRenderTarget.texture;
    composer.render();
  }

  function setSize(width, height) {
    composer.setSize(width, height);
    occlusionRenderTarget.setSize(width, height);
  }

  return { render, setSize };
}
